...
#define ALIGNED_FREE(ptr) free((ptr))
    ...
    void ha_init(void)
    {
        ha_coveredColumns = (uint8_t *)ALIGNED_ALLOC(ALIGNMENT, tk_max_dets * sizeof(uint8_t));
        if (ha_coveredColumns == NULL)
            err_sys("alloc error ha_coveredColumns");

        ha_coveredRows = (uint8_t *)ALIGNED_ALLOC(ALIGNMENT, tk_max_tracks * sizeof(uint8_t));
        if (ha_coveredRows == NULL)
            err_sys("alloc error ha_coveredRows");

        ha_starMatrix = (uint8_t *)ALIGNED_ALLOC(ALIGNMENT, tk_max_tracks * tk_max_dets * sizeof(uint8_t));
        if (ha_starMatrix == NULL)
            err_sys("alloc error ha_starMatrix");

        ha_primeMatrix = (uint8_t *)ALIGNED_ALLOC(ALIGNMENT, tk_max_tracks * tk_max_dets * sizeof(uint8_t));
        if (ha_primeMatrix == NULL)
            err_sys("alloc error ha_primeMatrix");

        ha_newStarMatrix = (uint8_t *)ALIGNED_ALLOC(ALIGNMENT, tk_max_tracks * tk_max_dets * sizeof(uint8_t));
        if (ha_newStarMatrix == NULL)
            err_sys("alloc error ha_newStarMatrix");

        ha_distMatrixIn = (ELEM_T *)ALIGNED_ALLOC(ALIGNMENT, tk_max_tracks * tk_max_dets * sizeof(uint8_t));
    ...
    void assignmentoptimal(index_t *assignment, ELEM_T *cost, ELEM_T *distMatrix, index_t nOfRows, index_t nOfColumns)
    {
        ...
        *cost = 0;
        ...
        /* memory allocation */
        memset(ha_coveredColumns, 0, nOfColumns * sizeof(uint8_t));
        memset(ha_coveredRows, 0, nOfRows * sizeof(uint8_t));
        memset(ha_starMatrix, 0, nOfElements * sizeof(uint8_t));
        memset(ha_primeMatrix, 0, nOfElements * sizeof(uint8_t));
        memset(ha_newStarMatrix, 0, nOfElements * sizeof(uint8_t));
            ...
            for (row = 0; row < nOfRows; row++)
                for (col = 0; col < nOfColumns; col++)
                    if (fabs(distMatrix[row + nOfRows * col]) < ELEM_EPSILON)
                        if (!ha_coveredColumns[col])
                        {
                            ha_starMatrix[row + nOfRows * col] = 1;
                            ha_coveredColumns[col] = 1;
                            break;
                        }
        }
        else /* if(nOfRows > nOfColumns) */
        {
            ...
            for (col = 0; col < nOfColumns; col++)
                for (row = 0; row < nOfRows; row++)
                    if (fabs(distMatrix[row + nOfRows * col]) < ELEM_EPSILON)
                        if (!ha_coveredRows[row])
                        {
                            ha_starMatrix[row + nOfRows * col] = 1;
                            ha_coveredColumns[col] = 1;
                            ha_coveredRows[row] = 1;
                            break;
                        }
            for (row = 0; row < nOfRows; row++)
                ha_coveredRows[row] = 0;
        }
    ...
    static void step3(index_t *assignment, ELEM_T *distMatrix, uint8_t *starMatrix, uint8_t *newStarMatrix, uint8_t *primeMatrix, uint8_t *coveredColumns, uint8_t *coveredRows, index_t nOfRows, index_t nOfColumns, index_t minDim)
    {
        ...
        while (zerosFound)
        {
            ...
            for (col = 0; col < nOfColumns; col++)
                if (!coveredColumns[col])
                    for (row = 0; row < nOfRows; row++)
                        if ((!coveredRows[row]) && (fabs(distMatrix[row + nOfRows * col]) < ELEM_EPSILON))
                        {
                            /* prime zero */
