...
//********************************************************//
double HungarianAlgorithm::Solve(ELEM_T *DistMatrix, index_t *Assignment, index_t trk_num, index_t det_num)
{
	unsigned int nRows = trk_num;
	unsigned int nCols = det_num;
	...
	for (unsigned int i = 0; i < nRows; i++)
		for (unsigned int j = 0; j < nCols; j++)
			distMatrixIn[i *nCols + j] = DistMatrix[i * nCols + j];
	//memcpy(distMatrixIn, DistMatrix, nRows * nCols * sizeof(ELEM_T));
